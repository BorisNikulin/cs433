...
Taking into account large modern L1 cache sizes and the fact that the benchmark is only using 20 PCBs,
it is reasonable to expect most, if not all, accesses of the data to be an L1 cache hit.
