...
or \SI{1}{\ms}.
Adding requires, at worst, traversing through $\log_2(20) = 4.32$ elements and potentially branching.
A branch miss prediction is \SI{7}{\ns}.
If all 4.32 comparisons are miss predicted that would be $7 \cdot 4.32 = \SI{30.24}{\ms}$
plus the L1 cache hits of \SI{4.32}{\ms}.
The current total is \SI{35.56}{\ms}.
Although only 50\% of the time one adds,
this still shows that the rough estimate of the time should be in the 50's of \si{\ms}.

This very rough ballpark estimate shows that around \SI{50}{\ms} should be expected and is what the benchmark got.
